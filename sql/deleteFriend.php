...
if(empty($friendid) or $friendid == $personid) {
  ...
  if(!empty($row[0])) {
    $result = mysqli_query($con,"DELETE FROM Friends WHERE Personid=$personid AND Friendid=$friendid");
    $result = mysqli_query($con,"DELETE FROM Friends WHERE Personid=$friendid AND Friendid=$personid");
    echo 0;
  }
  else {
    echo -1;
